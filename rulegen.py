...
class RuleGen:
    ...
    def __init__(self, vocab_list=dict, basename='analysis',
        ...
        self.enchant = ench.request_pwl_dict(vocab_list)
    ...
    def check_reversible_password(self, password):
        ...
        # Auskommentiert... "köln" wird zb als PW nicht akzeptiert
    ...
    def analyze_password(self,password, rules_queue=multiprocessing.Queue(), words_queue=multiprocessing.Queue(), unmatched_passwords = None):
        ...
        # Generate rules for words not in the dictionary
            ...
            if len(words) == 0 or len(words[0]["hashcat_rules"]) == 0:
                unmatched_passwords.append(password)

        words = [x for x in words if not len(x["hashcat_rules"]) == 0]
    ...
    def print_hashcat_rules(self, words, password, rules_queue, words_queue):
        ...
        for word in sorted(words, key=lambda word: len(word["hashcat_rules"][0])):
            ...
            for hashcat_rule in word["hashcat_rules"]:
                ...
                rule_length = len(hashcat_rule)
                #TODO: irgendwo hier fuckt es ab...
    ...
    def password_worker(self,i, passwords_queue, rules_queue, words_queue, unmatched_passwords):
        ...
        try:
            while True:
                ...
                self.analyze_password(password, rules_queue, words_queue, unmatched_passwords)
    ...
    def word_worker(self, words_queue, output_words_filename):
        """ Worker to store generated words. """
    ...
    def analyze_passwords_list(self, passwords_list):
        ...
        # List for unmatched passwords
        manager = multiprocessing.Manager()
        unmatched_passwords = manager.list([])

        # Start workers
        #for i in range(self.threads):
        multiprocessing.Process(target=self.password_worker, args=(0, passwords_queue, rules_queue, words_queue,
                                                                   unmatched_passwords)).start()
        ...
        try:
            ...
            with progressbar.ProgressBar(max_value=len(passwords_list)) as bar:
                i = 0
           ...
           # Wait for all of the queued passwords to finish.
            ...
            while not (rules_queue.empty() and words_queue.empty()):
                time.sleep(1)
        ...
        #    words_i += 1
        words = [x[:-1] for x in words_file]
        words_file.close()
        #words_sorted_file.close()

        unmatched = list(unmatched_passwords)

        return(words, rules, unmatched)
...
def main(passwordslist=list, maxworddist=10, maxwords=5, morewords=False, simplewords=False,
         maxrulelen=10, maxrules=5, morerules=False, simplerules=False, bruterules=False,
         word=None, verbose=False, debug=False, hashcat=False, quiet=True,
         wordlist=None, password=None,
         basename='analysis' , threads=multiprocessing.cpu_count()):

    if passwordslist is None:
        print("no passwords file specified")
        exit(1)
    if wordlist is None:
        print("Need Wordlist...")
        exit(1)
    rulegen = RuleGen(vocab_list=wordlist, basename=basename, threads=threads)

    # Finetuning word generation
    rulegen.max_word_dist=maxworddist
    rulegen.max_words=maxwords
    rulegen.more_words=morewords
    rulegen.simple_words=simplewords

    # Finetuning rule generation
    rulegen.max_rule_len=maxrulelen
    rulegen.max_rules=maxrules
    rulegen.more_rules=morerules
    rulegen.simple_rules=simplerules
    rulegen.brute_rules=bruterules
    if rulegen.brute_rules: print("[!] Bruteforcing reversal and rotation rules. (slower)")

    # Debugging options
    rulegen.word = word
    rulegen.verbose=verbose
    rulegen.debug = debug
    rulegen.hashcat = hashcat
    rulegen.quiet = quiet

    # Analyze a single password or several passwords in a file
    if password:
        rulegen.analyze_password(passwordsfile)
    else:
        return(rulegen.analyze_passwords_list(passwordslist))

    #print("missed Words: ", rulegen.unmatched_passwords)
